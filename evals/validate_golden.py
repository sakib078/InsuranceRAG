...
def locator_index() -> dict[str, list[str]]:
    """locator -> chunk_ids. Built from the chunks on disk, never from the manifest."""
    ...
    for path in sorted(CHUNKS_DIR.glob("*.jsonl")):
        ...
        for line in path.read_text(encoding="utf-8").splitlines():
            if line.strip():
                ...
                index.setdefault(chunk["locator"], []).append(chunk["chunk_id"])
    return index


def resolve(gold: str, index: dict[str, list[str]]) -> list[str]:
    """Exact match, or the ` #2` sub-chunks an oversized provision was split into."""
    hits = list(index.get(gold, []))
    for locator, ids in index.items():
        if locator.startswith(f"{gold} #"):
            hits.extend(ids)
    return hits


def check(records: list[dict], index: dict[str, list[str]]) -> list[str]:
...
def main() -> None:
    ...
    index = locator_index()
    problems = check(records, index)

    slices = Counter(r.get("hop") for r in records)
    print(f"{len(records)} records over {len(index)} distinct locators")
