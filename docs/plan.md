...
<details>
<summary><b>Phase 7 design — the build plan, not yet started</b></summary>
...
them at any depth, however it is ranked. Chunk size did not move this, a lexical channel did
not, two cross-encoders did not. The one variable never changed is the *shape*: one question,
one embedding, one pool. The agent changes what gets asked, and is the only remaining mechanism
that can pass 0.381.

Exclusion recall is 0.500 at k=5 and **0.731 at k=20** — so 0.731 is the number the mandatory
...
and check whether it comes back. This is the ceiling on *any* decomposition strategy — a
sub-question is at best a paraphrase of the provision it is meant to find.

**Kill criterion:** below ~0.75 self-retrieval, oracle decomposition caps multi-hop at roughly
`p^2.5` ≈ 0.28 — *below what one query already achieves*. Record Phase 7 as measured-and-dropped
beside Phases 5 and 6 and stop.
...
k=25, that control has to be re-measured as `pool_k25` before the agent has anything honest to
be compared against.
...
4. **`context_chunks` and `context_tokens` on every row.** Without them a long agent prompt gets
   compared to the baseline's measured 825 tokens and called a win — exactly how the uniform-512
   arm was caught in Phase 3.

### What gets built

**`insurance_rag/agent/tools.py`** — `fetch_neighbors(chunk_id, window)`, `expand(chunks,
window)`, `list_documents()`, and the exclusion pass. `chunk_id` is `f"{doc_id}:{ordinal:04d}"`
and `__post_init__` enforces it, so neighbour ids are computable; chunks are read from disk as
`evaluators._corpus()` already does. Ordinal adjacency is worth more than it looks: clustering
the 38 needed provisions by in-document position shows a ±2 window removes **6 of them** from
the set retrieval must independently hit, for zero tokens.

The exclusion pass fuses three channels — a template probe query unfiltered, the plain question
at `role_filter=[EXCLUSION, CONDITION]` on low weight, and cue-based promotion (gold exclusion
chunks carry limiting language at **0.73 against a 0.245 corpus base rate**). **Its top 2 hits
enter the context unconditionally**, regardless of fused rank: without a reserved floor, RRF can
outvote the mandatory check and the edge becomes decorative — the spec's own failure mode
relocated from the prompt into the ranker.

**`insurance_rag/agent/decompose.py`** — `mistral/ministral-14b-latest` via the existing
`providers.chat_model`, cached to `data/decompositions.jsonl` by question hash. 62 fixed
questions: ~16k Mistral tokens once, **0 Groq**, free on every later run. The cache doubles as
the inspection artefact.
...
`exclusion_check` is on an **unconditional** edge; the only conditional edge is after
`synthesize`. The retry trigger is **deterministic** — refusal string present, zero cited
locators, or the exclusion channel contributed nothing — never a second LLM call.

Reuse rather than fork: `format_context`, `within_budget`, `cited`, `REFUSAL`, `revoked_docs`,
...
**`tests/test_agent.py`** — `"exclusion" in checked` for all 62 records, which is what makes
step 4 an edge rather than a claim; refusal reachable and returning exactly `REFUSAL` with zero
citations; `steps <= max_agent_steps`; `retries <= 1`; the budget guard raising before Groq's
cap rather than after.

**New config keys:** `agent_context_tokens` (3000, half of `MAX_CONTEXT_TOKENS`, so the
comparison is budget-matched), `agent_neighbor_window` (2), `agent_max_sub_questions` (3 — mean
independent provisions per failing record is 2.5), `agent_token_budget` (185_000, to fail before
Groq's 200k rather than after), `decompose_model`.
...
| Web fallback | **dropped** — `enable_web_fallback` stays `False` |
...
2. `validate_golden` → `all locators resolve`, 62 records.
3. `pytest tests/test_agent.py` with synthesis mocked — 0 tokens.
4. **Retrieval row before generation row.** The retrieval row is free and must match the
   ...
   measured.
5. Only then the generation row, ~135–150k Groq tokens at `agent_context_tokens=3000`.
6. Diff `misses` against the single-shot control record by record. The 13 names are known.
