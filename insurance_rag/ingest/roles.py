...
_TRIGGERS: tuple[tuple[ChunkRole, tuple[str, ...]], ...] = (
    (ChunkRole.EXCLUSION, (
        ...
        "excluded", "exclusion", "not entitled to",
    )),
    (ChunkRole.CONDITION, (
...
_HEADING_ROLES: tuple[tuple[ChunkRole, tuple[str, ...]], ...] = (
    (ChunkRole.DEFINITION, ("definition", "interpretation", "meaning of")),
    (ChunkRole.EXCLUSION, ("exclusion", "won't cover", "not covered", "not payable")),
    (ChunkRole.CONDITION, ("statutory condition", "condition", "duties", "obligation")),
    (ChunkRole.SCHEDULE, ("schedule", "table", "amount", "rate", "indexation")),
...
def classify_role(text: str, ancestor_path: tuple[str, ...], locator: str) -> ChunkRole:
    """Heading first, trigger phrases second, `other` when neither fires."""
    if _TERM_RE.search(text) or "“" in locator:
        return ChunkRole.DEFINITION

    headings = " ".join(ancestor_path).lower()
    for role, needles in _HEADING_ROLES:
        if any(n in headings for n in needles):
            return role
